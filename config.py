class Config:
    ...
    use_txtfile = True
    pot_train_mode = 2
    reverse_distance_transform = False
    output_stride = 8
    ...
    def init_extra(self):
        ...
        elif self.DATASET == 'CustomPotSeg':
    ...
    def merge_from_args(self, args):
        ...
        if args.DEBUG is not None: self.DEBUG = args.DEBUG
        if args.TEST_N_EPOCHS is not None: self.TEST_N_EPOCHS = args.TEST_N_EPOCHS
        if args.testValTrain is not None: self.testValTrain = args.testValTrain
        if args.ignore_loss_index is not None: self.ignore_loss_index = args.ignore_loss_index
        if args.ignore_index is not None: self.ignore_index = args.ignore_index
        if args.rotate_degree is not None: self.rotate_degree = args.rotate_degree
        if args.base_size is not None: self.base_size = args.base_size
        if args.crop_size is not None: self.crop_size = args.crop_size
        if args.use_txtfile is not None: self.use_txtfile = args.use_txtfile
        if args.pot_train_mode is not None: self.pot_train_mode = args.pot_train_mode
        if args.base_size is not None and self.DATASET == 'CustomPotSeg':
            self.INPUT_WIDTH = args.base_size
            self.INPUT_HEIGHT = args.base_size
        if args.reverse_distance_transform is not None: self.reverse_distance_transform = args.reverse_distance_transform
    ...
    def get_as_dict(self):
        params = {
            ...
            "DILATE": self.DILATE,
            "DEBUG": self.DEBUG,
            "TEST_N_EPOCHS": self.TEST_N_EPOCHS,
            "N_CLS_SEG": self.N_CLS_SEG,
            
            "testValTrain": self.testValTrain,
            "ignore_loss_index": self.ignore_loss_index,
            "ignore_index": self.ignore_index,
            "rotate_degree": self.rotate_degree,
            "base_size": self.base_size,
            "crop_size": self.crop_size,
            "use_txtfile": self.use_txtfile,
            "pot_train_mode": self.pot_train_mode,
            "reverse_distance_transform":self.reverse_distance_transform,
...
def load_from_dict(dictionary):
    ...
    cfg.ignore_loss_index = dictionary.get("ignore_loss_index", None)
    cfg.ignore_index = dictionary.get("ignore_index", None)
    cfg.rotate_degree = dictionary.get("rotate_degree", None)
    cfg.base_size = dictionary.get("base_size", None)
    cfg.crop_size = dictionary.get("crop_size", None)
    cfg.use_txtfile = dictionary.get("use_txtfile", None)
    cfg.pot_train_mode = dictionary.get("pot_train_mode", None)
    cfg.reverse_distance_transform = dictionary.get('reverse_distance_transform', None)
    cfg.output_stride = dictionary.get('output_stride', None)
    cfg.de_ignore_index = dictionary.get('de_ignore_index', None)
