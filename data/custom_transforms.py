...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if 'seg_loss_mask' in sample:
            ...
            if isinstance(seg_loss_mask, Image.Image):
                seg_loss_mask = np.array(seg_loss_mask)
            if seg_loss_mask.ndim == 2:
                seg_loss_mask = np.expand_dims(seg_loss_mask, axis=0)    
    ...
    def __call__(self, sample):
        ...
        if random.random() < 0.25:
            ...
            if 'seg_loss_mask' in sample:
                ...
                seg_loss_mask = seg_loss_mask.transpose(Image.FLIP_LEFT_RIGHT)
                sample['seg_loss_mask'] = seg_loss_mask
    ...
    def __call__(self, sample):
        ...
        if random.random() < 0.25:
            ...
            if 'seg_loss_mask' in sample:
                seg_loss_mask = sample['seg_loss_mask']
                assert isinstance(seg_loss_mask, Image.Image)
                seg_loss_mask = seg_loss_mask.transpose(Image.FLIP_TOP_BOTTOM)
                sample['seg_loss_mask'] = seg_loss_mask
        # return {'image': img,
        #         'label': mask}
        sample['image'] = img
    ...
    def __call__(self, sample):
        ...
        if random.random() < 0.25:
            ...
            if 'seg_loss_mask' in sample:
                seg_loss_mask = sample['seg_loss_mask']
                assert isinstance(seg_loss_mask, Image.Image)
                seg_loss_mask = seg_loss_mask.rotate(rotate_degree, Image.BILINEAR, fillcolor = self.ignore_loss_index)
                sample['seg_loss_mask'] = seg_loss_mask
...
class CenterPadAndCrop(object):
    ...
    def __call__(self, sample):
        ...
        if 'seg_loss_mask' in sample:
            seg_loss_mask = sample['seg_loss_mask']
            assert isinstance(seg_loss_mask, Image.Image)
            seg_loss_mask = ImageOps.expand(seg_loss_mask, border=(left, top, right, bottom), fill=self.ignore_loss_index)
            seg_loss_mask = seg_loss_mask.crop((x1, y1, x1 + self.crop_size, y1 + self.crop_size))
            sample['seg_loss_mask'] = seg_loss_mask
...
class ShortEdgeCrop(object):
    ...
    def __call__(self, sample):
        ...
        if 'seg_loss_mask' in sample:
            seg_loss_mask = sample['seg_loss_mask']
            assert isinstance(seg_loss_mask, Image.Image)
            seg_loss_mask = seg_loss_mask.crop((x1, y1, x1 + ow, y1 + oh))
            sample['seg_loss_mask'] = seg_loss_mask
...
class RandomCutPostives(object):
    ...
    def __call__(self, sample):
        ...
        if 'seg_loss_mask' in sample:
            seg_loss_mask = sample['seg_loss_mask']
            assert isinstance(seg_loss_mask, Image.Image)
            seg_loss_mask = seg_loss_mask.crop((x1, y1, x2, y2))            
            sample['seg_loss_mask'] = seg_loss_mask
...
class FixedResize(object):
    def __init__(self, size, args = None, cfg = None):
    ...
    def __call__(self, sample):
        ...
        if self.cfg is None:
            ...
            mask = mask.resize((self.size[0]//self.cfg.output_stride, self.size[1]//self.cfg.output_stride), Image.NEAREST)
        if 'seg_loss_mask' in sample:
            seg_loss_mask = sample['seg_loss_mask']
            assert isinstance(seg_loss_mask, Image.Image)
            if self.cfg is None:
                seg_loss_mask = seg_loss_mask.resize(self.size, Image.NEAREST)
            else:
                seg_loss_mask = seg_loss_mask.resize((self.size[0]//self.cfg.output_stride, self.size[1]//self.cfg.output_stride), Image.NEAREST)
            sample['seg_loss_mask'] = seg_loss_mask
